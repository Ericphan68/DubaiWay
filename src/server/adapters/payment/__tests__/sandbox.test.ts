...
describe('Cổng thanh toán — chống trùng (idempotency)', () => {
  it('gọi hai lần cùng khoá chỉ tạo MỘT giao dịch', async () => {
    const input = { bookingId: 'b1', amount: usd(315), description: 'Safari', idempotencyKey: 'key-abc' };
  ...
  it('khoá khác nhau tạo giao dịch khác nhau', async () => {
    const a = await sandboxGateway.createIntent({ bookingId: 'b1', amount: usd(100), description: 'x', idempotencyKey: 'k1' });
    const b = await sandboxGateway.createIntent({ bookingId: 'b2', amount: usd(100), description: 'x', idempotencyKey: 'k2' });
...
describe('Hoàn tiền', () => {
  it('không hoàn quá số đã thu', async () => {
    const i = await sandboxGateway.createIntent({ bookingId: 'b1', amount: usd(300), description: 'x', idempotencyKey: 'k1' });
    await expect(
      sandboxGateway.refund({ paymentIntentId: i.id, amount: usd(400), idempotencyKey: 'r1' }),
    ).rejects.toThrow(/nhiều hơn số tiền còn lại/);
  });

  it('hoàn nhiều lần cộng dồn không vượt tổng', async () => {
    const i = await sandboxGateway.createIntent({ bookingId: 'b1', amount: usd(300), description: 'x', idempotencyKey: 'k1' });
    await sandboxGateway.refund({ paymentIntentId: i.id, amount: usd(200), idempotencyKey: 'r1' });
    await sandboxGateway.refund({ paymentIntentId: i.id, amount: usd(100), idempotencyKey: 'r2' });
    await expect(
      sandboxGateway.refund({ paymentIntentId: i.id, amount: usd(1), idempotencyKey: 'r3' }),
    ).rejects.toThrow(/nhiều hơn số tiền còn lại/);
  });

  it('giao dịch không tồn tại → báo lỗi', async () => {
    await expect(
      sandboxGateway.refund({ paymentIntentId: 'khong-co', amount: usd(1), idempotencyKey: 'r' }),
...
describe('Trạng thái giao dịch', () => {
  it('mới tạo thì chờ khách thao tác, chưa phải đã trả', async () => {
    const i = await sandboxGateway.createIntent({ bookingId: 'b1', amount: usd(100), description: 'x', idempotencyKey: 'k1' });
