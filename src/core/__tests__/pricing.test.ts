...
/** 1 AED = 100 fils. Helper cho dễ đọc test. */
const aed = (v: number) => fromMajorUnits(v, 'AED');

describe('Ví dụ chuẩn trong yêu cầu: đơn 1.000 AED', () => {
  const fin = computeBookingFinancials({
    currency: 'AED',
    lines: [{ label: 'Người lớn', unitPrice: aed(1000), quantity: 1 }],
    hasReferrer: true,
  });

  it('khách trả 1.000 AED', () => {
    expect(fin.customerTotal).toEqual(aed(1000));
    expect(formatMoney(fin.customerTotal, 'en-AE')).toContain('1,000.00');
  });

  it('hoa hồng DubaiWay = 100 AED (10%)', () => {
    expect(fin.platformCommission).toEqual(aed(100));
  });

  it('Merchant nhận 900 AED', () => {
    expect(fin.merchantRevenue).toEqual(aed(900));
  });

  it('người giới thiệu nhận 30 AED = 30% CỦA HOA HỒNG, không phải 30% đơn hàng', () => {
    expect(fin.referralReward).toEqual(aed(30));
    // 30% của giá trị đơn hàng sẽ là 300 AED — phải KHÁC
    expect(fin.referralReward).not.toEqual(aed(300));
  });

  it('DubaiWay thực giữ 70 AED', () => {
    expect(fin.platformNetRevenue).toEqual(aed(70));
  });
...
describe('Không có người giới thiệu', () => {
  it('thưởng = 0, nền tảng giữ trọn hoa hồng', () => {
    ...
    expect(fin.platformNetRevenue).toEqual(aed(100));
  });
});
...
describe('Nhiều dòng giá, giảm giá, thuế và phí', () => {
  const fin = computeBookingFinancials({
    currency: 'AED',
    lines: [
      { label: 'Người lớn', unitPrice: aed(350), quantity: 2 }, // 700
      { label: 'Trẻ em', unitPrice: aed(175), quantity: 2 }, // 350
    ],
    discounts: [aed(50)],
    taxes: [aed(50)], // VAT 5% thu hộ
    fees: [aed(20)], // phí dịch vụ nền tảng
    hasReferrer: true,
  });

  it('tạm tính 1.050, sau giảm giá 1.000', () => {
    expect(fin.subtotal).toEqual(aed(1050));
    expect(fin.discountTotal).toEqual(aed(50));
  });

  it('khách trả = 1.000 + 50 thuế + 20 phí = 1.070', () => {
    expect(fin.customerTotal).toEqual(aed(1070));
  });

  it('hoa hồng tính trên 1.000 (sau giảm giá, chưa gồm thuế/phí) = 100', () => {
    expect(fin.commissionBaseAmount).toEqual(aed(1000));
    expect(fin.platformCommission).toEqual(aed(100));
  });
  ...
  it('phí nền tảng là nguồn thu riêng, không chia cho merchant', () => {
    expect(fin.platformFeeRevenue).toEqual(aed(20));
  });

  it('thưởng giới thiệu = 30 AED', () => {
    expect(fin.referralReward).toEqual(aed(30));
  });
});
...
describe('Tỷ lệ có thể cấu hình, không ghi cứng', () => {
  it('đổi hoa hồng sang 15% và thưởng sang 20%', () => {
    const fin = computeBookingFinancials(
      { currency: 'AED', lines: [{ label: 'x', unitPrice: aed(1000), quantity: 1 }], hasReferrer: true },
      { commissionRateBps: 1500, referralShareBps: 2000, commissionBase: 'subtotal_after_discount' },
    );
    expect(fin.platformCommission).toEqual(aed(150));
    expect(fin.merchantRevenue).toEqual(aed(850));
    expect(fin.referralReward).toEqual(aed(30)); // 20% của 150
    expect(fin.platformNetRevenue).toEqual(aed(120));
  });
  ...
  it('đổi cơ sở tính hoa hồng sang tổng khách trả', () => {
    const fin = computeBookingFinancials(
      {
        currency: 'AED',
        lines: [{ label: 'x', unitPrice: aed(1000), quantity: 1 }],
        taxes: [aed(50)],
        hasReferrer: false,
      },
      { ...DEFAULT_COMMISSION_CONFIG, commissionBase: 'customer_total' },
    );
    expect(fin.commissionBaseAmount).toEqual(aed(1050));
    expect(fin.platformCommission).toEqual(aed(105));
  });

  it('tỷ lệ được snapshot vào kết quả để lưu theo booking', () => {
    const fin = computeBookingFinancials({
      currency: 'AED',
      lines: [{ label: 'x', unitPrice: aed(100), quantity: 1 }],
    });
...
describe('Làm tròn với số lẻ', () => {
  it('đơn 333,33 AED: hoa hồng 33,33 và merchant 300,00 — tổng vẫn khớp', () => {
    const fin = computeBookingFinancials({
      currency: 'AED',
      lines: [{ label: 'x', unitPrice: aed(333.33), quantity: 1 }],
      hasReferrer: true,
    });
  ...
  it('đơn rất nhỏ 0,01 AED không làm âm tiền merchant', () => {
    const fin = computeBookingFinancials({
      currency: 'AED',
      lines: [{ label: 'x', unitPrice: money(1, 'AED'), quantity: 1 }],
      hasReferrer: true,
    });
...
describe('Hoàn tiền — bút toán đảo', () => {
  ...
  it('hoàn toàn bộ: thu hồi đúng bằng số đã ghi nhận', () => {
    const adj = computeRefundAdjustment(fin, aed(1000));
    expect(adj.commissionReversal).toEqual(aed(-100));
    expect(adj.merchantRevenueReversal).toEqual(aed(-900));
    expect(adj.referralRewardReversal).toEqual(aed(-30));
    expect(adj.platformNetRevenueReversal).toEqual(aed(-70));
  });
  ...
  it('hoàn 50%: thu hồi đúng một nửa', () => {
    const adj = computeRefundAdjustment(fin, aed(500));
    expect(adj.commissionReversal).toEqual(aed(-50));
    expect(adj.merchantRevenueReversal).toEqual(aed(-450));
    expect(adj.referralRewardReversal).toEqual(aed(-15));
    expect(adj.platformNetRevenueReversal).toEqual(aed(-35));
  });
  ...
  it('không có referrer thì không thu hồi thưởng', () => {
    ...
    const adj = computeRefundAdjustment(noRef, aed(1000));
    expect(adj.referralRewardReversal.amount).toBe(0);
  });

  it('không cho hoàn quá số khách đã trả', () => {
    expect(() => computeRefundAdjustment(fin, aed(1001))).toThrow(/hoàn nhiều hơn/);
  });

  it('không cho hoàn số âm', () => {
    expect(() => computeRefundAdjustment(fin, money(-1, 'AED'))).toThrow(/không âm/);
  });
});
